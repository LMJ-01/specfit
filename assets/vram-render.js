 ...
 *   minipc   — 통합메모리 미니PC 도 같은 이유입니다 (본체째 사는 물건)
 *   new:false — 단종된 카드를 "이걸 사세요" 라고 권할 수 없습니다
 *
 * 넷 다 **드롭다운에는 남습니다.** 진단은 받아야 하기 때문입니다.
 */
const buyable = (g) => !g.mac && !g.laptop && !g.minipc && g.new;
...
<div class="vr">
  ...
  <p class="vr-note">
    ${gpu.mac ? '맥은 시스템이 메모리 일부를 쓰므로 전체 용량을 다 쓰지는 못합니다. 위 수치는 실제 사용 가능분 기준입니다. ' : ''}${gpu.laptop ? '노트북 GPU 는 같은 이름이어도 전력 제한이 제조사·모델마다 달라 <strong>속도가 갈립니다.</strong> 판정은 VRAM 기준이라 그대로 쓰실 수 있습니다. ' : ''}${gpu.minipc ? '통합메모리는 시스템과 나눠 쓰므로 위 수치는 그래픽 쪽에 내줄 수 있는 몫 기준입니다. 급 구분은 <a href="/posts/mini-pc-local-llm.html">미니PC 로컬 LLM 판정법</a>에 있습니다. ' : ''}
