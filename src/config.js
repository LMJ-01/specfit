...
export const config = {
  ...
  // ── 애드센스 심사 모드 (2026-09-01 도입) ────────────────────────
  //
  // true 면 빌드가 **모든 제휴 요소를 화면에서 뺍니다**:
  //   구매 박스({{BUY:}}) · 상품 위젯({{COUPANG:}}) · 홈 쿠팡 검색창 ·
  //   계산기 결과의 구매 버튼 · (요소가 없어지므로) 공정위 고지까지.
  // 글 본문·계산기 기능·링크 데이터는 그대로 남습니다 — 숨기는 것이지
  // 지우는 것이 아니고, false 로 되돌리면 전부 복원됩니다.
  //
  // 왜: 3차 거절(9/1)까지 색인 46+ 상태에서도 같은 일반 거절문 →
  // 색인 가설 기각. 남은 유력 용의자가 **제휴 요소 밀도**(쿠팡 링크
  // 334개, 전 글 구매 박스)라서, 심사 기간 동안 제휴를 숨기고 받는
  // 통용 방식을 씁니다. 근거: side-income-plan/docs/next-steps.md §2.
  //
  // ⚠️ **승인되면 즉시 false 로.** 이 플래그가 true 인 동안 쿠팡 수익은 0 입니다.
  adsenseReview: true,
